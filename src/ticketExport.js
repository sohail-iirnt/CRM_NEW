...
export const CSV_HEADERS = ['ticketId','ticketType','customerName','source','destinationCompany','destinationBranch','freightOrderNo','loadNumber','vehicleNumber','shed','itemCode','itemDescription','qty','prdDate','warehouseOrderNumber','position']
export const CSV_TEMPLATE_ROWS = [
  ...
  { ticketId:'CRM-0002', ticketType:'outbound', customerName:'Example Customer', source:'', destinationCompany:'', destinationBranch:'Pune', freightOrderNo:'FO-002', loadNumber:'LOAD-002', vehicleNumber:'MH02CD0000', shed:'P', itemCode:'ITEM-002', itemDescription:'Example outbound material', qty:'50', prdDate:'2026-08-30', warehouseOrderNumber:'WO-002', position:'P-12' }
]

const escapeCsv = value => `"${String(value ?? '').replaceAll('"','""')}"`
export function ticketsToCsv(tickets) {
  return [CSV_HEADERS, ...tickets.map(ticket => CSV_HEADERS.map(header => ticket[header] ?? ''))].map(row => row.map(escapeCsv).join(',')).join('\r\n')
}
...
export function printTicketsPdf(tickets) {
  const popup=window.open('', '_blank', 'width=1200,height=800'); if(!popup) throw new Error('Please allow pop-ups to export PDF.')
  const rows=tickets.map(ticket=>`<tr><td>${safe(ticket.ticketId||ticket.id)}</td><td>${safe(ticket.ticketType)}</td><td>${safe(ticket.customerName)}</td><td>${safe(ticket.destinationBranch||ticket.destinationCompany)}</td><td>${safe(ticket.qty)}</td><td>${safe(STATUS_LABELS[ticket.currentStatus]||ticket.currentStatus)}</td><td>${safe(formatTime(ticket.updatedAt||ticket.createdAt))}</td></tr>`).join('')
  popup.document.write(`<!doctype html><html><head><title>CRM SAIDHARA Ticket Export</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;color:#172033}h1{font-size:22px;margin:0 0 4px}p{font-size:11px;color:#667085;margin:0 0 16px}table{width:100%;border-collapse:collapse;font-size:9px}th,td{border:1px solid #d9dee8;padding:6px;text-align:left}th{background:#eef2f7;font-size:8px;text-transform:uppercase}@media print{button{display:none}}</style></head><body><h1>CRM SAIDHARA — Ticket Export</h1><p>Generated ${safe(new Date().toLocaleString())} · ${tickets.length} tickets</p><table><thead><tr><th>Ticket ID</th><th>Type</th><th>Customer</th><th>Destination</th><th>Qty</th><th>Current Status</th><th>Last Updated</th></tr></thead><tbody>${rows}</tbody></table><script>window.onload=()=>window.print()<\/script></body></html>`); popup.document.close()
}
const safe=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]))
const formatTime=value=>value?.toDate?value.toDate().toLocaleString():(value?new Date(value).toLocaleString():'—')
